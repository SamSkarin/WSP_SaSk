...
if ($pdo) {
    ...
    // Fungerar som ett grundkommando för alla andra variablar inom måsvingarna.
    if (isset($_POST['submit'])) {
        $user = $_POST['author']; //Vem som lagt upp inlägget.
        $headline = $_POST['title']; //Vad inläggets namn ska vara.
        $headline = trim($headline);

        $slug = slugify($headline); //Det förenklar meningen, och förhindrar att den ändras yttligare.

        $text = $_POST['message']; //Representerar hela inlägget som användaren lägger upp.

        $sql = 'INSERT INTO Post (User_ID, Slug, Headline, Text) VALUES ("'.$user.'", "'.$slug.'", "'.$headline.'", "'.$text.'")';
